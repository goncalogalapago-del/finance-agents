...
class SaxoCsvReadOnlyAdapter(ReadOnlyFinanceAdapter):
    ...
    def validate_read_only_scope(self) -> None:
        if not self._csv_dir.exists() or not self._csv_dir.is_dir():
            raise ValueError(f"Saxo CSV directory does not exist: {self._csv_dir}")
        accounts_path = self._csv_dir / "accounts.csv"
        if not accounts_path.exists():
            raise ValueError(f"Saxo CSV fallback requires accounts.csv at {accounts_path}")
    ...
    def _required_str(row: dict[str, Any], key: str) -> str:
        value = row.get(key)
        if value is None:
            raise ValueError(f"Saxo CSV row missing required column `{key}`")
        text = str(value).strip()
        if not text:
            raise ValueError(f"Saxo CSV row has empty value for required column `{key}`")
        return text
    ...
    def _decimal(raw: Any) -> Decimal:
        try:
            ...
            raise ValueError(f"Invalid decimal value `{raw}` in Saxo CSV input") from exc
    ...
    def _currency(raw: str) -> str:
        ...
        if len(currency) != 3:
            raise ValueError(f"Invalid currency code `{raw}` in Saxo CSV input")
        return currency
